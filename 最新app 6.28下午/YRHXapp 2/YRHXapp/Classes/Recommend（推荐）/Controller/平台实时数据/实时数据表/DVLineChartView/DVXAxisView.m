...
-(UIView *)scrollBgView2{
    if (!_scrollBgView2) {
        _scrollBgView2 = [[UIView alloc]initWithFrame:CGRectMake(self.bounds.size.width, 0, self.bounds.size.width-5, self.bounds.size.height)];
...
-(UIView *)scrollBgView1{
    if (!_scrollBgView1) {
        _scrollBgView1 = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width-5, self.bounds.size.height)];
...
//    CGFloat separateMargin = 45;    //横线分割线的间距
...
- (void)pointDidClicked:(UIButton *)sender {
    ...
    UIButton *pointButton = [self.pointButtonArray objectAtIndex:sender.tag];
	...
	if ([self.delegate respondsToSelector:@selector(xAxisView:didClickButtonAtIndex:)]) {
		[self.delegate xAxisView:self didClickButtonAtIndex:sender.tag];
	}
